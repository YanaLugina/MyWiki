...
Worker'ы порождаются с использованием метода child_process.fork(), так что они общаются с родителем по IPC и передают серверные обработчики событий вперед и назад.
